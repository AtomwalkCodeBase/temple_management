...
const PackagesPanel = ({
  ...
  const list = Array.isArray(selectedHall?.service_variation_list)
    ...
    : (Array.isArray(hall?.service_variation_list) ? hall.service_variation_list : []);

  const formatPackageType = (priceType) => {
    switch(priceType) {
      case 'HOURLY': return 'Hourly Package';
      case 'HALF_DAY': return 'Half Day Package';
      case 'FULL_DAY': return 'Full Day Package';
      default: return `${priceType} Package`;
    }
  };
  ...
  return (
    <>
      ...
      <Header>
        <HeaderLeft>
          <Title>{list.length} Package{list.length !== 1 ? 's' : ''}</Title>
      ...
      {list.length > 0 ? (
        <>
          <Grid className={
            list.length === 1 ? 'single-package' :
            list.length === 2 ? 'two-packages' :
            'multiple-packages'
          }>
            {list.slice(0, 6).map((pkg, index) => (
              <PackageCard key={pkg.id || index}>
                ...
                <PackageInfo>
                  <InfoItem>
                    <InfoIcon viewBox="0 0 24 24" fill="none" stroke="currentColor">
          ...
          {list.length > 6 && (
            <MorePackages>
              +{list.length - 6} more package{list.length - 6 !== 1 ? 's' : ''}
            </MorePackages>
          )}
        </>
      ) : (
        <EmptyState>
          ...
          <EmptyDescription>
            Create your first package to get started with pricing and availability
          </EmptyDescription>
